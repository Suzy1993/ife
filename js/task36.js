...
var commands = ['BUILD', 'GO', 'TUN LEF', 'TUN RIG', 'TUN BAC', 'TRA LEF','TRA TOP','TRA RIG','TRA BOT','MOV LEF','MOV TOP','MOV RIG','MOV BOT'];
...
textarea.onkeypress = function(e) {
	...
	if (key == 13) { // 输入的是回车
		...
		for (var i = 0; i < cmds.length; i++) { // 判断指令是否合法，不合法的指令给出提示（背景色为红色）
			...
			if (!(cmd.length == 1 && (cmd[0] == "GO" || cmd[0] == "BUILD") || cmd.length == 2 && (cmd[0] == "BRU" || cmd[0] == "GO" && cmd[1] >= '0' && cmd[1] <= '9') || cmd.length == 2 && commands.indexOf(cmd[0] + " " + cmd[1]) != -1 || cmd.length == 3 && commandWithParams.indexOf(cmd[0] + " " + cmd[1]) != -1 && cmd[2] >= '0' && cmd[2] <= '9'))
...
document.getElementsByTagName("button")[2].onclick = function() {
	for (var i = 0; i < 5; i++) {
		...
		if (document.getElementById("box" + x + y).innerHTML == "" && document.getElementById("box" + x + y).style.background == "")
			document.getElementById("box" + x + y).style.background = 'grey';
		else
			i--;
	}
}
	...
	switch(deg) {
		case "0":
			if (x == 0) 
				...
				x--;
				if (document.getElementById("box" + x + y).style.background != "")
					console.log("已经有墙了");
				else
					document.getElementById("box" + x + y).style.background = 'grey';
			}
